...
<script>
	...
	$(document).ready(function() {
		...
		if ($(window).width() < 514){
			...
			events: [
				<?php
					$sql = "SELECT booking.*,field.field_name FROM booking,field WHERE booking.field_id = field.field_id AND booking.cus_id = '$_SESSION[cus_id]';";
			...
			events: [
				<?php
					...
					while($row = mysql_fetch_array($result)){
						...
						$time_start = explode(":", $row['booking_time']);
						$time_end = explode(":", $row['booking_endtime']);
						if($row['booking_status'] != "T"){
							$booking_status = "ยังไม่อนุมัติ";
						}else{
							$booking_status = "อนุมัติแล้ว";
						}
			echo "{
					title: '$row[booking_activity] $row[field_name] $booking_status',
					start: new Date($day_array[0],$day_array[1],$day_array[2],$time_start[0],$time_start[1]),
					end: new Date($day_array[0],$day_array[1],$day_array[2],$time_end[0],$time_end[1]),
					allDay: false
				},";
